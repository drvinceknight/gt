...
[Auctions](/topics/auctions.html) page. Each question is worth 25 marks.
...
**(c)(i) Expected revenue.** [5]

The winner pays the second-highest value \(\min(v_1, v_2)\), so

\[
R = \mathbb{E}[\min(v_1, v_2)] = \tfrac{1}{3}.
\]

**(c)(ii) Winner's expected payoff.** [5]

The winner has value \(\max(v_1, v_2)\) and pays \(\min(v_1, v_2)\), so the
expected payoff is

\[
\mathbb{E}[\max(v_1, v_2) - \min(v_1, v_2)] = \mathbb{E}|v_1 - v_2| = \tfrac{1}{3}.
\]
...
**(b)(i) Expected revenue.** [5]

The winner pays \(\max(v_1, v_2)/2\), so

\[
R = \tfrac{1}{2}\,\mathbb{E}[\max(v_1, v_2)] = \tfrac{1}{2}\times\tfrac{2}{3} = \tfrac{1}{3}.
\]

**(b)(ii) Interim expected payoff.** [5]

A bidder with value \(v\) wins when the other's value is below \(v\), which has
probability \(v\), and then pays \(v/2\):

\[
(v - \tfrac{v}{2})\cdot v = \frac{v^2}{2}.
\]

**(b)(iii) Second-price revenue.** [4]
...
Both auctions award the item to the highest-value bidder and give a bidder with
value \(0\) zero expected surplus. Under these conditions a bidder's expected
payment is pinned down by their value alone, so the expected payments, and hence
the seller's revenue, coincide across the two formats.
...
For \(N\) independent uniform values \(\mathbb{E}[\max_i v_i] = \dfrac{N}{N+1}\),
...
second-price auction the winner pays the second-highest value, so the expected
revenue is the expected second-highest of \(N\) uniform values,
\(\dfrac{N-1}{N+1}\), the same as in part (c).
